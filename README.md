...
<head>
    ...
    <title>豪華旅程規劃與分帳儀表板</title>
    <!-- Tailwind CSS CDN -->
    <script src="https://cdn.tailwindcss.com"></script>
    <!-- Font Awesome CDN for icons (修正 integrity 屬性) -->
    <link rel="stylesheet"
    ...
    <style>
        ...
        /* 補上金色樣式 Utility（text-gold / bg-gold / border-gold） */
        .text-gold {
            color: var(--color-gold);
        }
        .bg-gold {
...
<body>
    ...
    <div class="max-w-4xl mx-auto p-4 sm:p-6 pb-20">
        <header class="text-center mb-6 pt-4">
            <h1 class="text-3xl sm:text-4xl font-black tracking-wider">東京黑金尊享之旅</h1>
            <p class="text-sm text-gray-400 mt-1">您的私人旅程儀表板</p>
        </header>
    ...
    <div id="ai-modal" class="fixed inset-0 bg-black bg-opacity-80 hidden items-center justify-center p-4 z-[100]">
        <div class="luxury-card w-full max-w-lg p-6 max-h-[90vh] overflow-y-auto">
            <h3 class="text-2xl font-bold text-center mb-4 text-gold">AI 行程規劃與查詢</h3>
            <p class="text-gray-400 text-sm mb-4">使用 Google 搜尋與 AI 來優化行程或獲取即時資訊。例如：`請幫我查一下銀座附近最高評價的和牛餐廳`。</p>
            <textarea id="ai-query-input" rows="3" placeholder="輸入您的查詢..." class="luxury-input w-full mb-3"></textarea>
            
            <div class="flex space-x-3 mb-4">
                <button id="ai-query-btn" class="luxury-btn flex-1 py-2 flex items-center justify-center" disabled>
                    <i class="fas fa-search mr-2"></i> 查詢 (需載入API Key)
                </button>
    ...
    <div id="edit-item-modal" class="fixed inset-0 bg-black bg-opacity-80 hidden items-center justify-center p-4 z-[100]">
        <div class="luxury-card w-full max-w-lg p-6 max-h-[90vh] overflow-y-auto">
            <h3 class="text-2xl font-bold text-center mb-4 text-gold">編輯行程項目</h3>
            <form id="item-edit-form" class="space-y-3">
                ...
                <input type="text" id="edit-item-desc" placeholder="行程描述" required class="luxury-input w-full">

                <input type="text" id="edit-item-link" placeholder="地圖連結或搜尋關鍵字 (選填)" class="luxury-input w-full">
                ...
                <div class="flex items-center justify-between">
                    <div class="flex items-center">
                        ...
                        <label for="edit-item-highlight" class="ml-2 text-sm text-gray-400">設為重要/提醒事項</label>
                    </div>
    ...
    <script>
        ...
        const apiKey = ""; // 留空，若你要真的串接 Gemini 再自行填入
        ...
        const PAYMENT_METHODS = ['現金', '信用卡', '西瓜卡/IC卡', '轉帳', '其他'];
        
        // Itinerary Categories
        const CATEGORIES = [
            { key: 'Attraction', name: '景點/地標', icon: 'fa-archway', color: 'text-blue-400' },
            { key: 'Dining', name: '餐飲/美食', icon: 'fa-utensils', color: 'text-red-400' },
            { key: 'Activity', name: '活動/體驗', icon: 'fa-dice-d20', color: 'text-green-400' },
            { key: 'Shopping', name: '購物/血拼', icon: 'fa-shopping-bag', color: 'text-purple-400' },
            { key: 'Travel', name: '交通/移動', icon: 'fa-car', color: 'text-yellow-400' },
            { key: 'Accommodation', name: '住宿/休憩', icon: 'fa-hotel', color: 'text-cyan-400' },
            { key: 'Other', name: '其他/備註', icon: 'fa-info-circle', color: 'text-gray-400' }
        ];
        ...
        const INITIAL_STATE = {
            ...
            members: ["A", "B", "C"],
            expenses: [
                { id: Date.now(), desc: '測試：A 支付 $60', amount: 60, payer: 'A', splitBy: ['A', 'B', 'C'], currency: 'JPY', paymentMethod: '現金', timestamp: Date.now() - 2000 },
                { id: Date.now() + 1, desc: '測試：B 支付 $30 (信用卡)', amount: 30, payer: 'B', splitBy: ['A', 'B', 'C'], currency: 'JPY', paymentMethod: '信用卡', timestamp: Date.now() - 1000 },
                { id: Date.now() + 2, desc: '測試：C 支付 $1000 台幣', amount: 1000, payer: 'C', splitBy: ['A', 'C'], currency: 'TWD', paymentMethod: '轉帳', timestamp: Date.now() },
            ],
            ...
            packingList: [
                ...
                { id: 2, text: '網卡/eSIM', checked: true },
                { id: 3, text: '金卡/信用卡', checked: false },
                { id: 4, text: '旅行文件', checked: true },
                { id: 5, text: '精緻服裝', checked: false },
                { id: 6, text: '現金 (日幣)', checked: true },
            ],
            memos: [
                { id: 1, text: '今天晚餐想吃壽喜燒' },
                { id: 2, text: '重要網址: https://example.com/tickets' },
            ],
            
            // --- ITINERARY DATA ---
            flightInfo: '去程：星宇航空 JX800 TPE(08:30) → NRT(12:30)；回程：星宇航空 JX801 NRT(18:00) → TPE(21:00)',
            accommodationInfo: '東京麗思卡爾頓酒店 (The Ritz-Carlton, Tokyo)，地址：9 Chome-7-1 Akasaka, Minato City, Tokyo',
            dailyItinerary: [
                { day: 'D1', date: 'Thu, Mar 12', items: [
                    { time: '12:30', desc: '抵達 NRT 機場', type: 'Travel', id: 101 },
                    { time: '13:30', desc: '抵達酒店辦理入住 (重要)', highlight: true, type: 'Accommodation', id: 102 },
                    { time: '16:00', desc: '六本木之丘展望台 (看日落)', link: 'https://maps.app.goo.gl/RohtvS69f9XpG5p26', type: 'Attraction', id: 103 },
                    { time: '19:30', desc: 'Azabu-Juban 豪華壽司晚餐', link: 'https://www.google.com/search?q=Azabu+Juban+luxury+sushi', type: 'Dining', id: 104 }
                ]},
                { day: 'D2', date: 'Fri, Mar 13', items: [
                    { time: '09:00', desc: '包車出發前往鎌倉 (重要)', highlight: true, type: 'Travel', id: 201 },
                    { time: '11:00', desc: '鎌倉大佛與長谷寺', link: 'https://maps.app.goo.gl/t9v4Wp8pGg9d4A3g6', type: 'Attraction', id: 202 },
                    { time: '14:00', desc: '湘南海岸散步', type: 'Activity', id: 203 },
                    { time: '18:00', desc: '返回東京，在銀座品嚐頂級和牛', link: 'https://www.google.com/search?q=Ginza+wagyu+restaurant', type: 'Dining', id: 204 }
                ]},
                { day: 'D3', date: 'Sat, Mar 14', items: [
                    { time: '10:00', desc: '明治神宮參拜', type: 'Attraction', id: 301 },
                    { time: '13:00', desc: '表參道精品購物', type: 'Shopping', id: 302 },
                    { time: '16:00', desc: '青山藝術區咖啡廳休息', type: 'Activity', id: 303 },
                    { time: '19:00', desc: '表參道米其林法餐', highlight: true, type: 'Dining', id: 304 }
                ]},
                { day: 'D4', date: 'Sun, Mar 15', items: [
                    { time: '11:00', desc: '築地場外市場尋覓美食', type: 'Dining', id: 401 },
                    { time: '15:00', desc: '銀座旗艦店自由購物', type: 'Shopping', id: 402 },
                    { time: '19:00', desc: '高級鐵板燒晚餐', type: 'Dining', id: 403 }
                ]},
                { day: 'D5', date: 'Mon, Mar 16', items: [
                    { time: '10:00', desc: '上野公園與東京國立博物館', type: 'Attraction', id: 501 },
                    { time: '14:00', desc: '淺草寺與仲見世通紀念品購買', type: 'Shopping', id: 502 },
                    { time: '18:00', desc: '返回酒店整理行李', highlight: true, type: 'Accommodation', id: 503 }
                ]},
                { day: 'D6', date: 'Tue, Mar 17', items: [
                    { time: '11:00', desc: '酒店 Check-out', highlight: true, type: 'Accommodation', id: 601 },
                    { time: '12:00', desc: '前往成田機場 (NRT)', type: 'Travel', id: 602 },
                    { time: '18:00', desc: '星宇航空 JX801 啟程回台', type: 'Travel', id: 603 }
                ]}
            ]
        };
        ...
        async function handleGeminiQuery(query) {
            ...
            const systemPrompt = "你是一位專業的旅遊規劃師，請根據我的查詢，以繁體中文提供精準且實用的旅遊建議。如果查詢需要外部資訊，請使用 Google Search 提供的資料，並在回覆中引用來源。";
            ...
            try {
                ...
                let generatedText = '抱歉，未能獲取有效的回覆結構。';
                let sources = [];
                ...
                if (sources.length > 0) {
                    sourcesEl.innerHTML = '<p class="font-bold mt-2">資料來源:</p>';
                    sources.forEach(s => {
                        sourcesEl.innerHTML += `<a href="${s.uri}" target="_blank" class="block text-blue-400 hover:text-blue-200 truncate">${s.title}</a>`;
                    });
                } else {
                    sourcesEl.innerHTML = '<p class="text-gray-600">此回覆無外部資料來源引用。</p>';
                }

            } catch (error) {
                console.error("Gemini API Error:", error);
                responseTextEl.textContent = `[錯誤] 呼叫 AI 服務失敗：${error.message}。請檢查 API Key 設定或網路連線。`;
                sourcesEl.innerHTML = '';
            } finally {
        ...
        function renderPlanTab() {
            ...
            appData.dailyItinerary.forEach(day => {
                let itemsHtml = day.items.map(item => {
                    ...
                    // 🔍 改良：若 link 是關鍵字，就用 link 當搜尋字；若是網址就直接開網址
                    ...
                    const linkButton = (item.link || searchQuery) ? `
                        <a href="${item.link && item.link.startsWith('http')
                           ...
                           target="_blank" class="text-xs text-blue-400 hover:text-blue-300 ml-2">
                            <i class="fas fa-map-marker-alt"></i> 地圖/食記
                        </a>
                    ` : '';
            ...
            const itineraryFormHtml = `
                <div class="luxury-card p-4 mb-6">
                    <h3 class="text-xl font-bold text-gold mb-3">📝 快速新增行程項目</h3>
                    <form id="itinerary-form" class="space-y-3">
                        <div class="flex space-x-3">
                            <select id="item-day" required class="luxury-input flex-1">
                                <option value="" disabled selected>選擇日期 (Day)</option>
                        ...
                        <select id="item-type" required class="luxury-input w-full">
                            <option value="" disabled selected>選擇分類 (景點/吃飯/交通)</option>
                            ${CATEGORIES.map(cat => `<option value="${cat.key}">${cat.name}</option>`).join('')}
                        </select>
                        
                        <input type="text" id="item-desc" placeholder="行程描述 (例如: 銀座和牛晚餐)" required class="luxury-input w-full">

                        <input type="text" id="item-link" placeholder="地圖連結或搜尋關鍵字 (選填)" class="luxury-input w-full">
                        
                        <div class="flex items-center">
                            <input type="checkbox" id="item-highlight" class="h-4 w-4 text-gold bg-gray-700 border-gray-600 rounded focus:ring-gold">
                            <label for="item-highlight" class="ml-2 text-sm text-gray-400">設為重要/提醒事項</label>
                        </div>

                        <button type="submit" class="luxury-btn w-full py-2">
                            <i class="fas fa-calendar-plus mr-2"></i> 新增行程
                        </button>
                    </form>
                </div>
            `;
            ...
            return `
                <section>
                    <div class="luxury-card p-4 mb-6">
                        <h2 class="text-2xl font-bold mb-3 text-gold">航班與住宿資訊</h2>
                    ...
                    <button id="open-ai-modal" class="luxury-btn w-full py-3 mb-6 flex items-center justify-center">
                        <i class="fas fa-brain mr-2"></i> AI 行程規劃與查詢 (Google 系統)
                    </button>
                    
                    ${itineraryFormHtml}

                    <h2 class="text-2xl font-bold mb-4 text-gold">每日行程表</h2>
                    ${itineraryHtml}
                </section>
            `;
        }
        ...
        function renderGuideTab() {
            return `
                <section>
                    <h2 class="text-2xl font-bold mb-4 text-gold text-center">深度導覽 - 奢華手冊</h2>
                    
                    <div class="luxury-card p-5 mb-6">
                        <h3 class="text-xl font-bold text-gold mb-3 border-b border-gray-700 pb-2">鎌倉大佛：東方的靜謐巨像</h3>
                        <p class="text-sm text-gray-300 mb-4">
                            這尊高達 13.35 公尺的阿彌陀佛像，是繼奈良大佛後日本第二大佛。它最初被安置在木造佛殿內，但在 15 世紀的海嘯中被沖毀，從此大佛便露天矗立至今。在黑金的現代設計中，我們仍能感受到它經歷風霜的莊嚴與靜謐。
                        </p>
                        
                        <h3 class="text-xl font-bold text-gold mb-3 border-b border-gray-700 pb-2">六本木之丘：東京的藝術心臟</h3>
                        <p class="text-sm text-gray-300 mb-4">
                            六本木之丘不僅是豪華購物中心，更是森美術館的所在地。站在展望台上，您可以鳥瞰整個東京，特別是在日落時分，從現代建築群中透出的金色夕陽，與我們的「黑金」主題完美呼應。
                        </p>
                    </div>

                    <div class="luxury-card p-5">
                        <h3 class="text-xl font-bold text-gold mb-3">青山/表參道區域地圖</h3>
                        ...
                        <small class="block text-center text-gray-400 mt-2">地圖中心為表參道站，供您導覽精品購物區。</small>
                    </div>
                </section>
            `;
        }
        ...
        function renderWalletTab() {
            ...
            const calculatorHtml = `
                <div class="luxury-card p-4 mb-6">
                    <h3 class="text-xl font-bold text-gold mb-3">💸 即時匯率換算 (JPY → TWD)</h3>
                    <div class="flex flex-col space-y-2 mb-3">
                        ...
                        <input type="text" id="calc-input" placeholder="輸入日幣算式 (e.g., 500+200*3)" class="luxury-input w-full">
                    </div>
                    <div class="flex justify-between items-center bg-black p-3 rounded-lg border border-gray-700">
                        <span class="text-sm text-gray-400">日幣總額 (JPY): <span id="calc-jpy" class="font-bold text-lg text-white">0</span></span>
                        <span class="text-sm text-gray-400">台幣換算 (TWD): <span id="calc-twd" class="font-bold text-lg text-gold">0</span></span>
                    </div>
                    <button id="calc-btn" class="luxury-btn w-full py-2 mt-3">計算</button>
                </div>
            `;
            
            const membersHtml = `
                <div class="luxury-card p-4 mb-6">
                    <h3 class="text-xl font-bold text-gold mb-3">👥 團員名單 (${appData.members.length} 人)</h3>
                    ...
                    <div class="flex space-x-2">
                        <input type="text" id="member-name-input" placeholder="新增團員姓名" class="luxury-input flex-grow">
                        <button id="add-member-btn" class="luxury-btn px-4 py-2 text-sm">新增</button>
                    </div>
                </div>
            `;
            ...
            const balanceHtml = `
                <div class="luxury-card p-4 mb-6">
                    <h3 class="text-xl font-bold text-gold mb-3">💰 團員結算餘額 (JPY)</h3>
                    <div id="balance-list" class="space-y-3">
                        ${appData.members.length === 0 ? '<p class="text-gray-500 text-center">請先新增團員。</p>' : 
                            Object.entries(balances).sort(([, a], [, b]) => b - a).map(([name, balance]) => {
                                ...
                                const sign = isOwes ? '應付 ' : (isOwed ? '應收 ' : '');
                                return `
                                    <div class="flex justify-between items-center p-2 rounded-lg bg-gray-700">
                                        ...
                                        <span class="text-lg font-extrabold ${balanceClass}">
                                            ${sign} ${jpyFormatter.format(Math.abs(finalBalance)).replace('¥', '')} JPY
                                        </span>
                                    </div>
                                `;
                            }).join('')
                    ...
                    <p class="mt-3 text-xs text-gray-400">正數：應收回 (付太多)；負數：應付回 (欠錢)</p>
                </div>
            `;
            
            const settlementHtml = `
                <div class="luxury-card p-4 mb-6 bg-yellow-900 bg-opacity-30 border-l-4 border-gold">
                    <h3 class="text-xl font-bold text-gold mb-3">✨ 最小轉帳建議 (JPY)</h3>
                    <div id="settlement-suggestions" class="space-y-3">
                        ${settlementData.length === 0 ? '<p class="text-center text-gray-400 py-2">所有餘額已結清。</p>' : 
                            settlementData.map(item => `
                                <div class="flex items-center justify-between p-2 rounded-lg bg-gray-800">
                                    ...
                                    <span class="text-lg font-extrabold text-gold">
                                        ${jpyFormatter.format(item.amount).replace('¥', '')} JPY
                                    </span>
                                </div>
                            `).join('')
                        }
            ...
            const expenseFormHtml = `
                <div class="luxury-card p-4 mb-6">
                    <h3 class="text-xl font-bold text-gold mb-3">📝 新增記帳紀錄</h3>
                    <form id="expense-form" class="space-y-3">
                        <input type="text" id="expense-desc" placeholder="品項描述" required class="luxury-input w-full">
                        
                        <div class="flex space-x-3">
                            <input type="number" id="expense-amount" placeholder="金額" step="0.01" required class="luxury-input flex-1">
                            ...
                            <select id="expense-payer" required class="luxury-input flex-1">
                                <option value="" disabled selected>誰付的錢？</option>
                        ...
                        <div class="p-3 bg-gray-800 rounded-lg">
                            <label class="block text-sm font-medium text-gray-300 mb-2">誰需要分攤？</label>
                            <div id="split-members-container" class="flex flex-wrap gap-x-4 gap-y-2">
                                ...
                                `).join('')}
                                ${appData.members.length === 0 ? '<p class="text-sm text-gray-500">請先新增團員</p>' : ''}
                            </div>
                        </div>
                        ...
                        <div class="flex space-x-3">
                            <button type="button" id="upload-photo-btn" class="luxury-btn py-2 flex-1 bg-gray-600 hover:bg-gray-700 text-white text-sm">
                                <i class="fas fa-camera mr-2"></i> 拍照/上傳收據
                            </button>
                            <button type="submit" class="luxury-btn py-2 flex-1 text-sm">
                                紀錄花費
                            </button>
                        </div>
                        <p id="photo-status" class="text-sm text-gold mt-2 hidden">已準備好圖片！</p>
            ...
            const expenseListHtml = `
                <div class="luxury-card p-4">
                    <h3 class="text-xl font-bold text-gold mb-3">💵 記帳歷史紀錄 (${appData.expenses.length} 筆)</h3>
                    <div id="expense-list" class="space-y-3 max-h-96 overflow-y-auto scroll-gold">
                        ${appData.expenses.length === 0 ? '<p class="text-center text-gray-500">尚無記帳紀錄。</p>' : 
                                ...
                                return `
                                    <div class="p-3 rounded-lg bg-gray-800 border-l-4 border-gray-600 flex justify-between items-center">
                                        <div class="flex items-start space-x-3 w-3/4">
                                            ${exp.photo ? `<img src="${exp.photo}" class="w-12 h-12 object-cover rounded-md shadow-lg" alt="收據縮圖">` : ''}
                                            <div class="flex-1 min-w-0">
                                                <p class="text-sm font-semibold text-white truncate">${exp.desc}</p>
                                                <p class="text-xs text-gray-400">支付者: ${exp.payer} | 分攤: ${exp.splitBy.join(', ')}</p>
                                                <p class="text-xs text-gray-500">方式: ${exp.paymentMethod || 'N/A'}</p>
                                            </div>
                                        </div>
                                        <div class="text-right w-1/4">
                                            <p class="text-base font-bold text-gold">
                                                ${mainFormatter.format(exp.amount).replace('¥', '').replace('NT$', '')} ${exp.currency}
                                            </p>
                                            <p class="text-xs text-gray-400">
                                                約 ${(equivalentCurrency === 'TWD' ? twdFormatter : jpyFormatter).format(equivalentAmount).replace('¥', '').replace('NT$', '')} ${equivalentCurrency}
                                            </p>
            ...
            return `
                <section>
                    <h2 class="text-2xl font-bold mb-4 text-gold text-center">WALLET - 旅費管理中心</h2>
                    ${calculatorHtml}
                    ${membersHtml}
                    ${balanceHtml}
                    ${settlementHtml}
        ...
        function renderListsTab() {
            const checklistHtml = `
                <div class="luxury-card p-4 mb-6">
                    <h3 class="text-xl font-bold text-gold mb-3 border-b border-gray-700 pb-2">✅ 行前準備清單</h3>
                    ...
                    <form id="add-packing-form" class="mt-4 flex space-x-2">
                        <input type="text" id="new-packing-item" placeholder="新增準備項目" class="luxury-input flex-grow">
            ...
            const memoHtml = `
                <div class="luxury-card p-4">
                    <h3 class="text-xl font-bold text-gold mb-3 border-b border-gray-700 pb-2">📌 個人備忘錄</h3>
                                    ...
                                    <div class="flex-1 min-w-0 pr-4">
                                        ...
                                        ${isUrl ? `
                                            <a href="${item.text}" target="_blank" class="mt-1 luxury-btn inline-block py-1 px-3 text-xs">
                                                <i class="fas fa-external-link-alt mr-1"></i> 點擊前往連結
                                            </a>
                                        ` : ''}
                                    </div>
                    ...
                    <form id="add-memo-form" class="mt-4 flex space-x-2">
                        <input type="text" id="new-memo-item" placeholder="新增備忘錄或網址" class="luxury-input flex-grow">
                        <button type="submit" class="luxury-btn px-3 py-2 text-sm"><i class="fas fa-plus"></i></button>
                    </form>
                </div>
            `;
            ...
            return `
                <section>
                    <h2 class="text-2xl font-bold mb-4 text-gold text-center">LISTS - 清單與備忘</h2>
                    ${checklistHtml}
                    ${memoHtml}
                </section>
            `;
        ...
        function renderInfoTab() {
            const emergencyContacts = [
                { name: '日本報警', number: '110', icon: 'fa-user-secret' },
                { name: '日本救護車/消防', number: '119', icon: 'fa-ambulance' },
                { name: '中華民國駐日代表處', number: '+81-3-3280-7800', icon: 'fa-flag-usa' }
            ];
            ...
            return `
                <section>
                    <h2 class="text-2xl font-bold mb-4 text-gold text-center">INFO - 重要旅遊資訊</h2>
                    
                    <div class="luxury-card p-4 mb-6 flex justify-between items-center bg-gray-800">
                        <p class="text-white font-semibold"><i class="fas fa-cloud-sun text-gold mr-2"></i> 當地天氣預報</p>
                        <a href="https://www.jma.go.jp/jma/index.html" target="_blank" class="luxury-btn py-1 px-3 text-xs">
                            日本氣象廳 <i class="fas fa-external-link-alt ml-1"></i>
                        </a>
                    </div>
                    
                    <div class="luxury-card p-4 mb-6">
                        <h3 class="text-xl font-bold text-gold mb-3 border-b border-gray-700 pb-2">🚨 緊急聯絡資訊</h3>
                        <div class="space-y-3">
                    ...
                    <div class="luxury-card p-4">
                        <h3 class="text-xl font-bold text-gold mb-3 border-b border-gray-700 pb-2">⚠️ 旅遊禁忌與注意事項</h3>
                        <ul class="list-disc list-inside space-y-2 text-sm text-gray-300">
                            ${prohibitions.map(item => `
                                <li class="text-white"><span class="text-gold font-bold mr-1">•</span> ${item}</li>
                            `).join('')}
                        </ul>
                        <p class="mt-4 text-xs text-gray-500">必備清單：${appData.packingList.map(i => i.text).join('、')}</p>
                    </div>
                </section>
            `;
        }
        ...
        function attachEventListeners(tabName) {
            ...
            if (tabName === 'plan') {
                ...
                if (apiKey !== "") { 
                    ...
                    aiQueryBtn.innerHTML = '<i class="fas fa-search mr-2"></i> 查詢';
                }
                ...
                aiQueryBtn.onclick = () => {
                    ...
                    if (query) {
                        ...
                        document.getElementById('ai-response-text').textContent = '請輸入查詢內容。';
                    }
                };
                ...
                document.getElementById('itinerary-form').onsubmit = (e) => {
                    ...
                    if (!dayKey || !time || !type || !desc) {
                        console.error('請填寫完整的日期、時間、分類和描述。');
                        return;
                    }
            ...
            if (tabName === 'wallet') {
                document.getElementById('calc-btn').onclick = () => {
                    ...
                    if (!/^[0-9+\-*/().\s]+$/.test(raw)) {
                        jpyEl.textContent = '錯誤';
                        twdEl.textContent = '只允許數字與 +-*/() 符號';
                        return;
                    }
                    ...
                    try {
                        ...
                        jpyEl.textContent = jpyFormatter.format(jpy).replace('¥', '');
                        twdEl.textContent = twdFormatter.format(round(jpy * appData.exchangeRate));
                    } catch (e) {
                        jpyEl.textContent = '錯誤';
                        twdEl.textContent = '請輸入有效算式';
                    }
                };
                ...
                photoInput.onchange = (e) => {
                    ...
                    if (file) {
                        imageToBase64(file, (base64) => {
                            ...
                            document.getElementById('photo-status').textContent = '已準備好圖片！';
                ...
                document.getElementById('expense-form').onsubmit = (e) => {
                    ...
                    if (!desc || isNaN(amount) || amount <= 0 || !payer || !paymentMethod || splitBy.length === 0) {
                        console.error('請輸入有效金額、描述、支付者、付款方式並選擇至少一位分攤者。');
                        return;
                    }
